...
export const rawTransaction = async (
  ...
  try {
    ...
    const transactionErrorMessage = transactionError(transactions, parameters);
    const msg = `${invokingResource} was unable to complete a transaction!\n${transactionErrorMessage}\n${err.message}`;

    console.error(msg);

    TriggerEvent('pgadapter:transaction-error', {
    ...
    if (typeof err === 'object') delete err.sqlMessage;
