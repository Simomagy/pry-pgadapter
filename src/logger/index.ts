import { pg_debug, pg_log_size, pg_slow_query_warning, pg_ui } from '../config';
import type { CFXCallback, CFXParameters } from '../types';
import { dbVersion } from '../database';

let loggerResource = '';
let loggerService = GetConvar('pg_logger_service', '');
...
export const logQuery = (
  ...
  if (
    executionTime >= pg_slow_query_warning ||
    (pg_debug && (!Array.isArray(pg_debug) || pg_debug.includes(invokingResource)))
  ) {
    console.log(
  ...
  else if (logStorage[invokingResource].length > pg_log_size) logStorage[invokingResource].splice(0, 1);

  logStorage[invokingResource].push({
    query,
    executionTime,
    date: Date.now(),
    slow: executionTime >= pg_slow_query_warning ? true : undefined,
  });
};
...
RegisterCommand(
  ...
  (source: number) => {
    ...
    emitNet(`pgadapter:openUi`, source, {
...
onNet(
  ...
  (data: {
    ...
    if (typeof data.resource !== 'string' || !IsPlayerAceAllowed(source as unknown as string, 'command.pgadapter')) return;

    if (data.search) data.search = data.search.toLowerCase();

    const resourceLog = logStorage[data.resource] ?? [];
    const filteredLog = data.search
      ? resourceLog.filter((q) => q.query.toLowerCase().includes(data.search))
      : resourceLog;

    const sort = data.sortBy && data.sortBy.length > 0 ? data.sortBy[0] : false;
    const startRow = data.pageIndex * 10;
    const queries = sort ? sortQueries(filteredLog, sort).slice(startRow, startRow + 10) : filteredLog.slice(startRow, startRow + 10);
    const pageCount = Math.ceil(filteredLog.length / 10);

    if (!queries) return;

    let resourceTime = 0;
    let resourceSlowQueries = 0;
    const resourceQueriesCount = filteredLog.length;

    for (let i = 0; i < resourceQueriesCount; i++) {
      const query = filteredLog[i];

      resourceTime += query.executionTime;
      if (query.slow) resourceSlowQueries += 1;
    }

    emitNet(`pgadapter:loadResource`, source, {
      queries,
      pageCount,
