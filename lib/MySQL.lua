...
for i = 1, GetNumResourceMetadata(resourceName, 'pg_option') do
	local option = GetResourceMetadata(resourceName, 'pg_option', i - 1)
	options[option] = true
end
...
local pgadapter = exports['pry-pgadapter']

local pg_method_mt = {
	__call = function(self, query, parameters, cb)
		query, parameters, cb = safeArgs(query, parameters, cb, self.method == 'transaction')
		return pgadapter[self.method](nil, query, parameters, cb, resourceName, options.return_callback_errors)
	end
}

local PG = setmetatable(PG or {}, {
	__index = function(_, index)
		return function(...)
			return pgadapter[index](nil, ...)
		end
	end
})
...
local alias_mt = {
	__index = function(self, key)
		if alias[key] then
			local method = PG[alias[key]]
			PG.Async[key] = method
			PG.Sync[key] = method.await
			alias[key] = nil
			return self[key]
		end
	end
...
PG.Sync = setmetatable({ store = addStore }, alias_mt)
PG.Async = setmetatable({ store = addStore }, alias_mt)

local function onReady(cb)
	while GetResourceState('pry-pgadapter') ~= 'started' do
		Wait(50)
	end
...
function PG.startTransaction(cb)
	return pgadapter:startTransaction(cb, resourceName)
end
